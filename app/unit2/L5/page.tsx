        ...
        <div className="bg-card p-6 rounded-xl border border-border my-8">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
                    <ShieldCheck size={20} className={method === 'scanf' ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"} />
                    {method === 'scanf' ? 'The Validation Gate (scanf)' : 'The Robust Way (fgets)'}
                </h3>
                <button
                    onClick={() => setMethod(m => m === 'scanf' ? 'fgets' : 'scanf')}
                    className="text-xs bg-muted px-3 py-1 rounded-full border border-border hover:bg-muted/80 transition-colors text-foreground"
                >
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    <div className="bg-muted/50 p-4 rounded-xl border border-border">
                        <label className="text-xs font-bold text-muted-foreground uppercase block mb-2">Code Pattern</label>
                        {method === 'scanf' ? (
                    ...
                    <div className="flex gap-2">
                        <input
                            ...
                            className="bg-background border border-border rounded p-2 text-foreground flex-1 font-mono"
                        />
                        <button
                            onClick={processInput}
                            className="bg-blue-600 dark:text-white px-4 rounded font-bold hover:bg-blue-500"
                        >
                            Enter
                        </button>
                    </div>
                ...
                <div className="space-y-4">
                    <div className="bg-black/80 dark:bg-black/50 p-4 rounded-xl border border-border relative overflow-hidden min-h-[160px]">
                        {/* Status */}
                        ...
                        <div className="flex items-center gap-2 mb-2">
                            <span className="text-xs text-slate-400 uppercase">Stdin Buffer:</span>
                            <div className="flex gap-1 flex-wrap">
                                ...
                                {inputBuffer.length === 0 && <span className="text-xs text-slate-500 italic">Empty</span>}
                            </div>
                        </div>
    ...
    return (
        <div className="bg-card p-6 rounded-xl border border-border my-8">
            <h3 className="text-lg font-bold text-foreground mb-6 flex items-center gap-2">
                <Terminal size={20} className="text-green-600 dark:text-green-400" /> Unformatted Stream (getchar/putchar)
            </h3>

            <div className="grid md:grid-cols-3 gap-4 mb-6">
                <button onClick={() => setMode('upper')} className={`p-2 rounded border text-sm font-bold ${mode === 'upper' ? 'bg-muted border-foreground dark:border-white text-foreground' : 'bg-background border-border text-muted-foreground'}`}>To Upper</button>
                <button onClick={() => setMode('nospace')} className={`p-2 rounded border text-sm font-bold ${mode === 'nospace' ? 'bg-muted border-foreground dark:border-white text-foreground' : 'bg-background border-border text-muted-foreground'}`}>Strip Spaces</button>
                <button onClick={() => setMode('count')} className={`p-2 rounded border text-sm font-bold ${mode === 'count' ? 'bg-muted border-foreground dark:border-white text-foreground' : 'bg-background border-border text-muted-foreground'}`}>Word Count</button>
            </div>
            ...
            <div className="bg-black/90 p-4 rounded-xl border border-border font-mono min-h-[100px] relative">
                <div className="absolute top-2 right-2 text-[10px] text-zinc-500 uppercase font-bold">Output Stream</div>
                <div className="text-green-400 text-lg">
                    {output}<span className="animate-pulse">_</span>
                </div>
            </div>

            <div className="mt-4 p-3 bg-muted rounded border border-border text-xs text-muted-foreground font-mono">
...
const StateMachine = () => {
    ...
    return (
        <div className="bg-card p-6 rounded-xl border border-border my-8">
            <h3 className="text-lg font-bold text-foreground mb-6 flex items-center gap-2">
                <Activity size={20} className="text-purple-600 dark:text-purple-400" /> Structured Design: State Machine
            </h3>

            <div className="grid md:grid-cols-2 gap-8 items-center">
                {/* Visualizer */}
                <div className="relative h-64 border-2 border-border rounded-xl bg-card/50 flex items-center justify-center p-4">
            ...
            ${state === 'IDLE' ? 'border-muted-foreground text-muted-foreground bg-muted' : ''}
            ${state === 'RUNNING' ? 'border-green-500 text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/20 shadow-green-500/50 animate-pulse' : ''}
            ${state === 'PAUSED' ? 'border-yellow-500 text-yellow-600 dark:text-yellow-400 bg-yellow-100 dark:bg-yellow-900/20' : ''}
            ${state === 'ERROR' ? 'border-red-500 text-red-600 dark:text-red-500 bg-red-100 dark:bg-red-900/20 animate-bounce' : ''}
          `}>
                        {state}
                    </div>

                    <div className="absolute top-2 left-2 text-xs text-muted-foreground font-bold uppercase">Current State</div>
                </div>
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        <button onClick={() => transition('PAUSE')} className="bg-yellow-600 text-white p-2 rounded text-xs font-bold hover:bg-yellow-500">PAUSE</button>
                        <button onClick={() => transition('RESUME')} className="bg-blue-600 text-white p-2 rounded text-xs font-bold hover:bg-blue-500">RESUME</button>
                        <button onClick={() => transition('STOP')} className="bg-zinc-600 text-white p-2 rounded text-xs font-bold hover:bg-zinc-500">STOP</button>
                        <button onClick={() => transition('FAIL')} className="bg-red-100 dark:bg-red-900/50 border border-red-500 text-red-600 dark:text-red-400 p-2 rounded text-xs font-bold hover:bg-red-200 dark:hover:bg-red-900">TRIGGER ERROR</button>
                        <button onClick={() => transition('RESET')} className="bg-purple-600 text-white p-2 rounded text-xs font-bold hover:bg-purple-500">RESET SYSTEM</button>
                    </div>

                    <div className="bg-black/90 p-3 rounded-lg border border-border text-xs font-mono text-slate-400 h-32 overflow-hidden">
            ...
            <TheoryCard title="Pattern: switch inside while" variant="purple">
                ...
                <code className="block mt-2 bg-muted p-2 rounded text-xs text-foreground">
                    while(1) {'{'}<br />
                    &nbsp;&nbsp;switch(state) {'{'}<br />
                    &nbsp;&nbsp;&nbsp;&nbsp;case IDLE: ... break;<br />
...
const ModularDesignLab = () => {
    ...
    return (
        <div className="bg-card p-6 rounded-xl border border-border my-8">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
                    <LayoutTemplate size={20} className="text-orange-600 dark:text-orange-400" /> Structured: Top-Down Design
                </h3>
                <div className="flex gap-2">
                    <button onClick={() => setView('spaghetti')} className={`px-3 py-1 rounded text-xs font-bold border transition-colors ${view === 'spaghetti' ? 'bg-red-100 dark:bg-red-900/50 border-red-500 text-red-600 dark:text-red-200' : 'bg-muted border-border text-muted-foreground'}`}>Spaghetti Code</button>
                    <button onClick={() => setView('modular')} className={`px-3 py-1 rounded text-xs font-bold border transition-colors ${view === 'modular' ? 'bg-green-100 dark:bg-green-900/50 border-green-500 text-green-600 dark:text-green-200' : 'bg-muted border-border text-muted-foreground'}`}>Modular Functions</button>
                </div>
            </div>
          ...
          ${view === 'spaghetti' ? 'border-red-500 bg-red-100 dark:bg-red-900/10' : 'border-green-500 bg-green-100 dark:bg-green-900/10'}
        `}>
                    {view === 'spaghetti' ? (
                        <div className="text-center animate-in fade-in">
                            <div className="text-6xl mb-2">🍝</div>
                            <h4 className="text-red-500 font-bold">The Monolith</h4>
                            <p className="text-xs text-red-400 mt-2">1000 lines inside main().<br />Hard to read. Hard to debug.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-2 gap-4 w-full animate-in fade-in zoom-in">
                            <div className="bg-muted p-2 rounded text-center border border-green-500/30 text-green-600 dark:text-green-300 font-bold text-xs">Main()</div>
                            <div className="bg-muted p-2 rounded text-center border border-green-500/30 text-green-600 dark:text-green-300 font-bold text-xs">Input()</div>
                            <div className="bg-muted p-2 rounded text-center border border-green-500/30 text-green-600 dark:text-green-300 font-bold text-xs">Process()</div>
                            <div className="bg-muted p-2 rounded text-center border border-green-500/30 text-green-600 dark:text-green-300 font-bold text-xs">Display()</div>
...
const CleanCodeStudio = () => {
    ...
    return (
        <div className="bg-card p-6 rounded-xl border border-border my-8">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
                    <Scissors size={20} className="text-yellow-600 dark:text-yellow-400" /> Clean Code: Magic Numbers
                </h3>
                <button
                    onClick={() => setRefactored(!refactored)}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition-all ${refactored ? 'bg-green-600 text-white' : 'bg-muted text-muted-foreground'}`}
                >
                    ...
                    <TheoryCard title="Why avoid Magic Numbers?" variant="yellow">
                        <ul className="space-y-2 text-sm text-foreground/80">
                            <li className="flex items-start gap-2">
                                <Hash size={16} className="text-yellow-600 dark:text-yellow-500 mt-1" />
    ...
    return (
        <div className="bg-card p-6 rounded-xl border border-border my-8">
            <h3 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
                <Layers size={20} className="text-orange-600 dark:text-orange-400" /> Variable Scope & Lifetime
            </h3>

            <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-2">
                    <div className="bg-muted p-3 rounded-t-xl border border-border border-b-0 relative">
                        <span className="text-xs text-muted-foreground uppercase font-bold">Main Scope</span>
                        <code className="block text-blue-600 dark:text-blue-300 mt-1">int x = 10;</code>

                        <div className="mt-4 bg-card/80 p-3 rounded border border-border relative ml-4">
                            <span className="text-xs text-muted-foreground uppercase font-bold">Loop Scope (Block)</span>
                            <code className="block text-green-600 dark:text-green-300 mt-1">int x = 20;</code>
                            <code className="block text-muted-foreground text-xs mt-1">// Hides outer 'x'</code>
                        </div>

                        <code className="block text-blue-600 dark:text-blue-300 mt-4">printf("%d", x); // Prints 10</code>
                    </div>
                </div>
            ...
            <header className="fixed top-0 left-0 right-0 h-16 bg-background/90 backdrop-blur-md border-b border-border z-50 flex items-center justify-between px-6 md:px-12">
                <div className="flex items-center gap-3">
                    ...
                    <div className="hidden md:block">
                        <h1 className="font-bold text-foreground text-sm leading-tight">Robust I/O & Design</h1>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-widest">Unit 2 • Lecture 5 (Capstone)</p>
                    </div>
                </div>
            ...
            <main className="pt-32 px-6 md:px-12 max-w-7xl mx-auto space-y-24">
                ...
                <div className="text-center space-y-6">
                    ...
                    <h1 className="text-5xl md:text-7xl font-extrabold text-foreground tracking-tight">
                        Bulletproof <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-cyan-600 to-foreground dark:from-blue-400 dark:via-cyan-400 dark:to-white">Logic</span>
                    </h1>
